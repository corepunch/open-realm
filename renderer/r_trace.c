...
static vec2_t R_PointToViewSpace(viewDef_t const *viewdef, float x, float y) {
...
line3_t R_LineForScreenPoint(viewDef_t const *viewdef, float x, float y) {
    ...
    vec2_t const p = R_PointToViewSpace(viewdef, x, y);
    line3_t const line = {
        Matrix4_multiply_vector3(&invproj, &(vec3_t const) { p.x, p.y, 0 }),
        Matrix4_multiply_vector3(&invproj, &(vec3_t const) { p.x, p.y, 1 }),
    };
    return line;
}
...
uint32_t R_EntitiesInRect(viewDef_t const *viewdef, rect_t const *rect, uint32_t max, uint32_t *array) {
    ...
    tr.viewDef = *viewdef;
    vec2_t const a = R_PointToViewSpace(viewdef, rect->x, rect->y);
    vec2_t const b = R_PointToViewSpace(viewdef, rect->x+rect->w, rect->y+rect->h);
    rect_t const screen = {
        .x = MIN(a.x, b.x),
        .y = MIN(a.y, b.y),
