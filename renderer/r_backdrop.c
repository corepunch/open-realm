...
static RECT backdrop_edge_uv(BACKDROPCORNER c, FLOAT tile, int idx, BOOL wow) {
    if (wow) {
        /* WoW border textures are a 2x2 atlas; each edge/corner uses one
         * fixed quadrant. Repeating the full texture would wrap into the
         * neighboring quadrants and corrupt the border art. */
        switch (c) {
            case BACKDROP_LEFT_EDGE:   return (RECT){ 0.0f, 0.0f, 0.5f, 1.0f };
            case BACKDROP_RIGHT_EDGE:  return (RECT){ 0.5f, 0.0f, 0.5f, 1.0f };
            case BACKDROP_TOP_EDGE:    return (RECT){ 0.0f, 0.0f, 1.0f, 0.5f };
            case BACKDROP_BOTTOM_EDGE: return (RECT){ 0.0f, 0.5f, 1.0f, 0.5f };
...
void R_DrawBackdrop(LPCDRAWBACKDROP db) {
    ...
    if (db->edge.texture) {
        ...
        FOR_LOOP(i, NUM_BACKDROP_CORNERS) {
            ...
            FLOAT tile = wow ? 1.0f : backdrop_edge_tile(rects + c, c, edgeSize.height / 1000.f);

            RECT const uv = backdrop_edge_uv(c, tile, i, wow);
            BOOL const flip = !wow && backdrop_edge_flip(c);

            if (!wow && tile > 1.0f) edge_repeat = true;
