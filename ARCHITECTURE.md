...
- Keep one data representation per shader concept. A caller with one light uploads one normal light entry; it does not select a zero-count fallback mode with parallel uniforms. Pack parameters that form one effect state into a struct, vector array, or matrix when that removes parallel uniform contracts without obscuring the schema. Exception branches require proof that the common path cannot represent the data.
