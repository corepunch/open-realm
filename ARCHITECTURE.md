...
5. **Entity-context layers**: The server can declare generic bindings such as context name, health, mana, and an optional recipient-filtered numeric hover detail. For `LAYER_WORLD_HOVER`, the client chooses `cl.hover_entity`, projects its model-authored overhead point every render frame, and resolves those values from the existing `entityState_t` snapshot and `CS_GENERAL` pool. Mouse movement and projection do not make a server round trip; the server still owns the frame tree, labels, bindings, and visibility.
...
| `svc_layout` | `uint8_t layer`, `uiFrame_t[]`, optional compact payloads | Server-authored UI frame tree for a specific HUD layer. |
...
| `svc_configstring` | 16-bit index, string or fixed binary statusbar slot | Media registry and map metadata. |
| `svc_loading_screen` | 32-bit total, offset, and size, then compressed bytes | Chunked loading frame tree, sent after its media; complete receipt presents the screen. |
...
| Sound System & Entity Audio Architecture | [architecture/sound.md](architecture/sound.md) |
