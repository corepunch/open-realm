...
void Matrix4_lookAt(mat4_t *m, vec3_t const *eye, vec3_t const *direction, vec3_t const *up) {
    vec3_t zaxis = Vector3_unm(direction);
    vec3_t xyz = Vector3_unm(eye);
    vec3_t xaxis = Vector3_cross(up, &zaxis);
    vec3_t yaxis = Vector3_cross(&zaxis, &xaxis);
...
vec3_t Matrix4_multiply_vector3(mat4_t const *m, vec3_t const *v) {
...
void Matrix4_rotate(mat4_t *m, vec3_t const *euler, ROTATIONORDER order) {
    float const DEG2RAD = 3.14159f / 180.f;
    
    mat4_t rx, ry, rz, tmp, tmp2;
