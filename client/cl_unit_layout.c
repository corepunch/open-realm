...
static BOOL SCR_LayoutShouldSkipLayoutLayer(DWORD layer) {
    DWORD state = uiimport.GetPlayerState()->client_ui_state;
    if (state != CLIENT_UI_CINEMATIC && state != CLIENT_UI_LOADING) return false;
    switch (layer) {
        case LAYER_PORTRAIT: case LAYER_CONSOLE: case LAYER_COMMANDBAR:
        case LAYER_INFOPANEL: case LAYER_INVENTORY: return true;
...
static LPCENTITYSTATE SCR_LayoutSelectedEntity(void) {
    DWORD n = uiimport.GetNumEntities();
    FOR_LOOP(i, n) {
        LPCENTITYSTATE ent = uiimport.GetEntity(i);
...
static void SCR_LayoutDrawHighlightData(uiHighlight_t const *h, LPCRECT screen) {
    ...
    re.DrawImageEx(&MAKE(drawImage_t,
        .texture   = uiimport.GetTexture(h->alphaFile),
...
void SCR_LayoutSimpleButton(LPCUIFRAME frame, LPCRECT screen) {
    ...
    re.DrawText(&MAKE(drawText_t,
        .rect      = *screen,
        .font      = uiimport.GetFont(b->normal.font),
        .text      = frame->text,
...
void SCR_LayoutDrawBackdrop2(LPCUIFRAME frame, LPCRECT screen, uiBackdrop_t const *bd) {
    ...
    re.DrawBackdrop(&MAKE(drawBackdrop_t,
        .screen        = *screen,
        .bg.texture    = uiimport.GetTexture(bd->Background),
        .bg.color      = frame->color,
        .edge.texture  = uiimport.GetTexture(bd->EdgeFile),
...
void SCR_LayoutDrawMultiSelect(LPCUIFRAME frame, LPCRECT scrn) {
    ...
    FOR_LOOP(i, ms->numitems) {
        ...
        if (ent) {
            ...
            uv.w = hp;
            re.DrawImage(uiimport.GetTexture(ms->hp_bar),   &rect, &uv, MAKE(COLOR32,0,255,0,255));
            uv.w  = mana; rect.w  = screen.w * mana;
            rect.y += screen.h * (HP_BAR_HEIGHT_RATIO + HP_BAR_SPACING_RATIO);
            re.DrawImage(uiimport.GetTexture(ms->mana_bar), &rect, &uv, MAKE(COLOR32,0,255,255,255));
        }
...
void SCR_LayoutDrawPortrait(LPCUIFRAME frame, LPCRECT screen) {
    ...
    re.SetEntityAnimFrame(draw, "Stand", &entity);
...
void SCR_LayoutDrawListBox(LPCUIFRAME frame, LPCRECT screen) {
    ...
    while (line && item_y > list_rect.y) {
        ...
        re.DrawText(&MAKE(drawText_t,
            .font       = uiimport.GetFont(lb->text.font),
            .text       = line,
            .color      = frame->color.a ? frame->color : COLOR32_WHITE,
            .halign     = FONT_JUSTIFYLEFT,
...
void SCR_LayoutDrawFrame(LPCUIFRAME frame) {
    ...
    FOR_LOOP(j, sizeof(drawers)/sizeof(*drawers)) {
        if (drawers[j].type == frame->flags.type) { drawers[j].func(frame, screen); break; }
    }
}
...
void SCR_LayoutUpdateFrame(LPCUIFRAME frame) {
    ...
    FOR_LOOP(j, sizeof(updaters)/sizeof(*updaters)) {
        if (updaters[j].type == frame->flags.type) { updaters[j].func(frame, screen); break; }
    }
}
...
void SCR_DrawLayout(void) {
    ...
    FOR_LOOP(layer, MAX_LAYOUT_LAYERS) {
        DWORD flags = uiimport.GetPlayerState()->uiflags;
        if ((1 << layer) & flags || SCR_LayoutShouldSkipLayoutLayer(layer)) continue;
        HANDLE layout = layout_layers[layer];
        if (layout) { SCR_Clear(layout); SCR_LayoutUpdateTooltip(layout); }
    }
    ...
    FOR_LOOP(layer, MAX_LAYOUT_LAYERS) {
        ...
        if ((1 << layer) & flags || SCR_LayoutShouldSkipLayoutLayer(layer)) continue;
        HANDLE layout = layout_layers[layer];
        if (layout) { SCR_Clear(layout); SCR_LayoutUpdateTooltip(layout); SCR_LayoutDrawOverlay(layout); }
    }
}
...
void SCR_LayoutMouseEvent(uiMouseEvent_t event, int x, int y, int32_t param) {
    ...
    FOR_LOOP(layer, MAX_LAYOUT_LAYERS) {
        HANDLE layout = layout_layers[layer];
        DWORD flags = uiimport.GetPlayerState()->uiflags;
        if (!layout || (1 << layer) & flags || SCR_LayoutShouldSkipLayoutLayer(layer)) continue;
        SCR_Clear(layout);
    ...
    FOR_LOOP(layer, MAX_LAYOUT_LAYERS) {
        ...
        for (DWORD i = SCR_NumFrames(); i > 0; i--) {
            ...
            if (Rect_contains(SCR_LayoutRect(frame), &point)) {
                ...
                if (uiimport.ServerCommand) uiimport.ServerCommand(command);
                return;
            }
        }
    }
