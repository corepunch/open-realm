...
static inline void M_ModelMatrix(uiModel_t const *model, float aspect, mat4_t *out) {
    mat4_t proj, view, local;
    vec3_t dir = Vector3_sub(&model->target, &model->eye);
    ...
    Matrix4_lookAt(&view, &model->eye, &dir, &(vec3_t){ 0, 0, 1 });
    ...
    Matrix4_translate(&local, &(vec3_t){ model->pos.x * half_x, model->pos.z, model->pos.y * half_y });
