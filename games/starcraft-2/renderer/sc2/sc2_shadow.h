...
static bool sc2_shadow_matrix(sc2shadowview_t const *in, mat4_t *out) {
    mat4_t inv, view, proj;
    vec3_t dir = in->light;
    float radius = 0;
    Matrix4_inverse(&in->camera, &inv);
    FOR_LOOP(i, 4) {
        vec3_t a = Matrix4_multiply_vector3(&inv, &(vec3_t){ i & 1 ? 1 : -1, i & 2 ? 1 : -1, -1 });
        vec3_t b = Matrix4_multiply_vector3(&inv, &(vec3_t){ i & 1 ? 1 : -1, i & 2 ? 1 : -1, 1 });
        vec3_t ray = Vector3_sub(&b, &a);
        if (fabsf(ray.z) < 0.000001f) return false;
        vec3_t ground = Vector3_mad(&a, (in->target.z - a.z) / ray.z, &ray);
        radius = MAX(radius, Vector3_distance(&ground, &in->target));
    }
    if (radius <= 0 || Vector3_lengthsq(&dir) <= 0 || in->reach <= 0) return false;
    Vector3_normalize(&dir);
    vec3_t eye = Vector3_mad(&in->target, -in->reach, &dir);
    /* A vertical sun needs a non-parallel up vector to define its light basis. */
    vec3_t up = fabsf(dir.z) > 0.99f ? (vec3_t){ 0, 1, 0 } : (vec3_t){ 0, 0, 1 };
