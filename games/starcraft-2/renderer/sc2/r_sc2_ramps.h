...
static inline float r_sc2_ground_triangle_height(float const h[4], vector2_t p) {
...
static inline bool r_sc2_ramp_contains(sc2RampBox_t const *box, vector2_t p) {
    vector2_t d = Vector2_sub(&p, &box->center);
...
static inline box2_t r_sc2_ramp_bounds(sc2RampBox_t const *box) {
    vector2_t half = { fabsf(box->right.x) * box->width + fabsf(box->up.x) * box->height,
...
static inline uint32_t r_sc2_ramp_sample(sc2Map_t const *map, vector2_t p) {
...
static inline uint32_t r_sc2_ramp_level(sc2Map_t const *map, vector2_t p) {
...
static inline sc2RampPiece_t r_sc2_ramp_piece(sc2Map_t const *map, sc2Ramp_t const *ramp, uint32_t edge) {
    ...
    vector2_t corner[] = { piece.bounds.min, {piece.bounds.max.x, piece.bounds.min.y},
        piece.bounds.max, {piece.bounds.min.x, piece.bounds.max.y} };
    char raw[4];
    FOR_LOOP(i, 4) {
        if (r_sc2_ramp_contains(&ramp->mid, corner[i])) {
            vector2_t d = Vector2_sub(&corner[i], &ramp->mid.center);
...
static inline bool r_sc2_ramp_covers_ground(sc2Map_t const *map, vector2_t p) {
    vector2_t center = { floorf(p.x/2)*2+1, floorf(p.y/2)*2+1 };
    FOR_EACH_ARRAY(sc2Ramp_t, ramp, map->t3Terrain.ramps) {
        vector2_t d = Vector2_sub(&center, &ramp->mid.center);
