 ...
 * into uiBaseFrame_t arrays: anchor resolution, frame type mapping,
...
static uiBaseFrame_t *find_frame(uiBaseFrame_t *frames, DWORD count, LPCSTR name) {
...
static void test_adapter_template_inheritance_ordering(void) {
    ...
    ASSERT(button->has_width);
    ASSERT_EQ_FLOAT(button->width, 300.0f, 0.01f);
    ASSERT(button->has_height);
...
static void test_adapter_cross_file_template_inheritance(void) {
    ...
    ASSERT_NOT_NULL(panel);
    ASSERT(panel->has_width);
    ASSERT_EQ_FLOAT(panel->width, 100.0f, 0.01f);
    ASSERT(panel->has_height);
...
static void test_adapter_single_anchor_left_min(void) {
    ...
    uiBaseFrame_t *icon = find_frame(frames, count, "MineralIcon");
...
static void test_adapter_dual_anchor_stretch(void) {
    ...
    uiBaseFrame_t *panel = find_frame(frames, count, "ResourcePanel");
    ASSERT_NOT_NULL(panel);
...
static void test_adapter_mid_anchor(void) {
    ...
    uiBaseFrame_t *alert = find_frame(frames, count, "CenterAlert");
...
static void test_adapter_flatten_types_mapped(void) {
    ...
    uiBaseFrame_t *root = find_frame(frames, count, "ConsoleUI");
    ASSERT_NOT_NULL(root);
    ASSERT_EQ_INT(root->type, FT_FRAME);

    /* Cmd01 (CommandButton) → FT_BUTTON */
    uiBaseFrame_t *cmd = find_frame(frames, count, "Cmd01");
    ASSERT_NOT_NULL(cmd);
    ...
    uiBaseFrame_t *label = find_frame(frames, count, "MineralCount");
    ASSERT_NOT_NULL(label);
    ASSERT_EQ_INT(label->type, FT_TEXT);

    /* Portrait (Image) → FT_SPRITE */
    uiBaseFrame_t *portrait = find_frame(frames, count, "Portrait");
...
static void test_adapter_flatten_hidden_flags(void) {
    ...
    uiBaseFrame_t *hidden = find_frame(frames, count, "HiddenPanel");
    ASSERT_NOT_NULL(hidden);
    ASSERT(hidden->hidden);
    ASSERT(hidden->ui_flags & UIFLAG_HIDDEN);
...
static void test_adapter_cross_frame_relative_index(void) {
    ...
    uiBaseFrame_t *cmd03 = find_frame(frames, count, "Cmd03");
    uiBaseFrame_t *cmd02 = find_frame(frames, count, "Cmd02");
...
static void test_adapter_hidden_flagged_for_skip(void) {
    ...
    /* HiddenPanel and CenterAlert should have UIFLAG_HIDDEN set */
    uiBaseFrame_t *hidden = find_frame(frames, count, "HiddenPanel");
    uiBaseFrame_t *alert = find_frame(frames, count, "CenterAlert");
    ASSERT_NOT_NULL(hidden);
    ASSERT_NOT_NULL(alert);
    ASSERT(hidden->ui_flags & UIFLAG_HIDDEN);
    ASSERT(alert->ui_flags & UIFLAG_HIDDEN);

    /* MineralIcon should NOT have UIFLAG_HIDDEN */
    uiBaseFrame_t *icon = find_frame(frames, count, "MineralIcon");
    ASSERT_NOT_NULL(icon);
    ASSERT(!(icon->ui_flags & UIFLAG_HIDDEN));
