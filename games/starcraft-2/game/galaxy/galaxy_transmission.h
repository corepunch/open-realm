...
static uint32_t sc2_TransmissionSource(LPJASS j)        { return jass_pushnullhandle(j, "transmissionsource"); }
static uint32_t sc2_TransmissionSourceFromModel(LPJASS j){ return jass_pushnullhandle(j, "transmissionsource"); }
static uint32_t sc2_TransmissionSourceFromUnit(LPJASS j) { return jass_pushnullhandle(j, "transmissionsource"); }
/* TransmissionSend: send a transmission to players; sleep if waitUntilDone and duration > 0. */
static uint32_t sc2_TransmissionSend(LPJASS j) {
...
static uint32_t sc2_TransmissionLastSent(LPJASS j)      { return jass_pushinteger(j, 0); }
static uint32_t sc2_TransmissionClear(LPJASS j)         { (void)j; return jass_pushnull(j); }
static uint32_t sc2_TransmissionClearAll(LPJASS j)      { (void)j; return jass_pushnull(j); }
static uint32_t sc2_TransmissionWait(LPJASS j) {
    float secs = jass_checknumber(j, 2);
    if (secs > 0.0f) jass_sleep(j, (uint32_t)(secs * 1000.0f));
    return jass_pushnull(j);
}
static uint32_t sc2_TransmissionSetOption(LPJASS j)     { (void)j; return jass_pushnull(j); }
