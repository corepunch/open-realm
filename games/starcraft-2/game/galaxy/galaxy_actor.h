...
typedef struct { int32_t parent, unit; bool live; } SC2SCOPE;
...
static int32_t sc2_scope_create(LPJASS j, int32_t parent, int32_t unit) {
...
static int32_t sc2_actor_h(LPJASS j, int idx) {
...
static uint32_t sc2_ActorCreate(LPJASS j) {
...
static uint32_t sc2_ActorSend(LPJASS j) {
...
static uint32_t sc2_ActorScopeFromUnit(LPJASS j) {
...
static uint32_t sc2_ActorFrom(LPJASS j) {
...
static uint32_t sc2_ActorScopeFrom(LPJASS j) {
    cstring_t ref = jass_checkstring(j, 1);
    if (ref && !strcmp(ref, "::LastCreated"))
...
static uint32_t sc2_ActorScopeFromActor(LPJASS j) {
    int32_t h = sc2_actor_h(j, 1);
...
static uint32_t sc2_ActorScopeKill(LPJASS j) {
...
static uint32_t sc2_ActorFromScope(LPJASS j)      { (void)j; return jass_pushnullhandle(j, "actor"); }
static uint32_t sc2_ActorRegionCreate(LPJASS j)   { (void)j; return jass_pushnullhandle(j, "actorscope"); }
static uint32_t sc2_ActorRegionSend(LPJASS j)     { (void)j; return jass_pushnull(j); }

static uint32_t sc2_AchievementAward(LPJASS j)              { (void)j; return jass_pushnull(j); }
static uint32_t sc2_AchievementErase(LPJASS j)              { (void)j; return jass_pushnull(j); }
static uint32_t sc2_AchievementPanelSetCategory(LPJASS j)   { (void)j; return jass_pushnull(j); }
static uint32_t sc2_AchievementPanelSetVisible(LPJASS j)    { (void)j; return jass_pushnull(j); }
static uint32_t sc2_AchievementPercentText(LPJASS j)        { (void)j; return jass_pushnull(j); }
static uint32_t sc2_AchievementTermQuantitySet(LPJASS j)    { (void)j; return jass_pushnull(j); }
static uint32_t sc2_AchievementsDisable(LPJASS j)           { (void)j; return jass_pushnull(j); }

static uint32_t sc2_BankExists(LPJASS j)                    { (void)j; return jass_pushboolean(j, false); }
static uint32_t sc2_BankKeyRemove(LPJASS j)                 { (void)j; return jass_pushnull(j); }
static uint32_t sc2_BankLastCreated(LPJASS j)               { (void)j; return jass_pushnullhandle(j, "bank"); }
static uint32_t sc2_BankLoad(LPJASS j)                      { (void)j; return jass_pushnullhandle(j, "bank"); }
static uint32_t sc2_BankSave(LPJASS j)                      { (void)j; return jass_pushnull(j); }
static uint32_t sc2_BankValueSetFromFlag(LPJASS j)          { (void)j; return jass_pushnull(j); }
static uint32_t sc2_BankValueSetFromInt(LPJASS j)           { (void)j; return jass_pushnull(j); }
static uint32_t sc2_BankValueSetFromString(LPJASS j)        { (void)j; return jass_pushnull(j); }
static uint32_t sc2_BankValueSetFromText(LPJASS j)          { (void)j; return jass_pushnull(j); }

static uint32_t sc2_ConversationDataResetNodeState(LPJASS j)   { (void)j; return jass_pushnull(j); }
static uint32_t sc2_ConversationDataResetStateValues(LPJASS j) { (void)j; return jass_pushnull(j); }
static uint32_t sc2_ConversationDataSaveNodeState(LPJASS j)    { (void)j; return jass_pushnull(j); }
static uint32_t sc2_ConversationDataSaveStateValues(LPJASS j)  { (void)j; return jass_pushnull(j); }
static uint32_t sc2_ConversationDataStateFixedValue(LPJASS j)  { (void)j; return jass_pushnull(j); }
static uint32_t sc2_ConversationDataStateGetValue(LPJASS j)    { (void)j; return jass_pushnull(j); }
static uint32_t sc2_ConversationDataStateIndex(LPJASS j)       { (void)j; return jass_pushinteger(j, 0); }
static uint32_t sc2_ConversationDataStateIndexCount(LPJASS j)  { (void)j; return jass_pushinteger(j, 0); }
/* Presentation fields come from the layered ConversationState catalog, retained by the game module. */
static uint32_t sc2_conversation_field(LPJASS j, cstring_t field) {
...
static uint32_t sc2_ConversationDataStateName(LPJASS j) { return sc2_conversation_field(j, "Name"); }
static uint32_t sc2_ConversationDataStateImagePath(LPJASS j) { return sc2_conversation_field(j, "ImagePath"); }
static uint32_t sc2_ConversationDataStateSetValue(LPJASS j)    { (void)j; return jass_pushnull(j); }
static uint32_t sc2_ConversationDataStateText(LPJASS j) {
    char field[128];
    snprintf(field, sizeof(field), "Text:%s", jass_checkstring(j, 2));
    return sc2_conversation_field(j, field);
}

static uint32_t sc2_TimerPause(LPJASS j)                    { (void)j; return jass_pushnull(j); }
