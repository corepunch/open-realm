...
static uint32_t sc2_StringExternal(LPJASS j)     { cstring_t s = jass_checkstring(j,1); return jass_pushstring(j, s ? s : ""); }
/* Galaxy uses a null result to terminate whitespace-delimited word iteration. */
static uint32_t sc2_StringWord(LPJASS j) {
...
static uint32_t sc2_IntToText(LPJASS j) {
    char text[32];
...
static uint32_t sc2_color(LPJASS j, uint32_t count) {
...
static uint32_t sc2_Color(LPJASS j) { return sc2_color(j, 3); }
static uint32_t sc2_ColorWithAlpha(LPJASS j) { return sc2_color(j, 4); }
static uint32_t sc2_GameTimeOfDayPause(LPJASS j) { (void)j; return jass_pushnull(j); }
static uint32_t sc2_GameTimeOfDaySet(LPJASS j)   { (void)j; return jass_pushnull(j); }
static uint32_t sc2_GameSetBackground(LPJASS j)  { (void)j; return jass_pushnull(j); }
static uint32_t sc2_GameSetLighting(LPJASS j)    { (void)j; return jass_pushnull(j); }
static uint32_t sc2_DifficultyEnabled(LPJASS j)  { return jass_pushboolean(j, false); }
static uint32_t sc2_DifficultyName(LPJASS j)     { return jass_pushstring(j, ""); }
static uint32_t sc2_DifficultyNameCampaign(LPJASS j) { return jass_pushstring(j, ""); }
static uint32_t sc2_AITimePause(LPJASS j)        { (void)j; return jass_pushnull(j); }
static uint32_t sc2_AIDisableAllScouting(LPJASS j)          { (void)j; return jass_pushnull(j); }
static uint32_t sc2_CampaignMode(LPJASS j)                  { (void)j; return jass_pushnull(j); }
static uint32_t sc2_DataTableSetString(LPJASS j)            { (void)j; return jass_pushnull(j); }
static uint32_t sc2_DataTableValueExists(LPJASS j)          { (void)j; return jass_pushboolean(j, false); }
static uint32_t sc2_FixedToInt(LPJASS j)  { return jass_pushinteger(j, (int32_t)jass_checknumber(j, 1)); }
static uint32_t sc2_FixedToString(LPJASS j) {
    char buf[48];
...
static uint32_t sc2_FormatNumber(LPJASS j) {
...
static uint32_t sc2_GameCheatAllow(LPJASS j)                { (void)j; return jass_pushnull(j); }
static uint32_t sc2_GameGetSpeedValue(LPJASS j)             { (void)j; return jass_pushnumber(j, 1.0f); }
static uint32_t sc2_GameIsDebugOptionSet(LPJASS j)          { (void)j; return jass_pushboolean(j, false); }
static uint32_t sc2_GameIsTestMap(LPJASS j)                 { (void)j; return jass_pushboolean(j, false); }
static uint32_t sc2_GameIsTransitionMap(LPJASS j)           { (void)j; return jass_pushboolean(j, false); }
static uint32_t sc2_GameMapIsBlizzard(LPJASS j)             { (void)j; return jass_pushboolean(j, false); }
static uint32_t sc2_GamePauseAllCharges(LPJASS j)           { (void)j; return jass_pushnull(j); }
static uint32_t sc2_GameSetSeedLocked(LPJASS j)             { (void)j; return jass_pushnull(j); }
static uint32_t sc2_GameSetSpeedLocked(LPJASS j)            { (void)j; return jass_pushnull(j); }
static uint32_t sc2_GameSetSpeedValue(LPJASS j)             { (void)j; return jass_pushnull(j); }
static uint32_t sc2_IntToFixed(LPJASS j) { return jass_pushnumber(j, (float)jass_checkinteger(j, 1)); }
static uint32_t sc2_IntToString(LPJASS j) {
    char buf[32];
    snprintf(buf, sizeof(buf), "%ld", (long)jass_checkinteger(j, 1));
    return jass_pushstring(j, buf);
}
/* Galaxy uses 1-based, inclusive string indices. */
static uint32_t sc2_StringSub(LPJASS j) {
...
static uint32_t sc2_StringReplaceWord(LPJASS j) {
...
static uint32_t sc2_StringToText(LPJASS j) {
    cstring_t s = jass_checkstring(j, 1);
    return jass_pushstring(j, s ? s : "");
}
static uint32_t sc2_TextCase(LPJASS j) {
    cstring_t src  = jass_checkstring(j, 1);
