...
static uint32_t MDLX_RibbonConvertStrip(trailVert_t const *strip, uint32_t nverts, vertex_t *out, uint32_t max)
{
    uint32_t n;
...
static vertex_t orphan_verts[TRAIL_MAX_EDGES * 6];
...
static void MDLX_FadeRibbonVertices(vertex_t *verts, uint32_t nverts, trail_t const *trail,
                                    float lifespan)
{
    int alive, i, write;
...
void MDLX_TickDetachedRibbons(void) {
    ...
    while (*link) {
        ...
        if (!R_TrailAdvance(&orphan->trail, (vec3_t){0}, (vec3_t){0}, (color32_t){0},
...
static void MDLX_RibbonWorldEdge(mdxModel_t const *model, mdxRibbonEmitter_t const *ribbon,
                                 mat4_t const *model_matrix, float heightAbove, float heightBelow,
                                 vec3_t *above, vec3_t *below)
{
    mat4_t world;
    vec3_t local_above = { 0, heightAbove, 0 };
    vec3_t local_below = { 0, -heightBelow, 0 };
