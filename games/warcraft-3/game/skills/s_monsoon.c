...
void monsoon_think(LPEDICT ent) {
    DWORD code = ent->class_id, rank = ent->resources;
    DWORD step = (DWORD)(S_SpellData(code, rank, 2) * 1000.0f);
...
BZ_ABILITY_PROC(CAbilityMonsoon) {
    DWORD code = call && call->item ? call->item->code : 0, rank;
    LPEDICT thinker;
    ...
    thinker->spawn_time = G_Time() + (DWORD)(S_SpellDuration(code, rank, false) * 1000.0f);
