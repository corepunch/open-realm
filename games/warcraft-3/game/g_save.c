...
field_t fields[] = {
    EDICTFIELD(class_id, F_INT), EDICTFIELD(variation, F_INT), EDICTFIELD(build_project, F_INT),
    EDICTFIELD(spawn_time, F_INT), EDICTFIELD(harvested_lumber, F_INT), EDICTFIELD(harvested_gold, F_INT),
    EDICTFIELD(heatmap2, F_INT), EDICTFIELD(peonsinside, F_INT), EDICTFIELD(aiflags, F_INT),
    EDICTFIELD(damage, F_INT), EDICTFIELD(collision, F_FLOAT), EDICTFIELD(s.origin, F_VECTOR),
    EDICTFIELD(construction.primary_builder, F_EDICT), EDICTFIELD(rally.entity, F_EDICT),
    EDICTFIELD(revival.producer, F_EDICT), EDICTFIELD(revival.queue_next, F_EDICT), EDICTFIELD(goldmine.mine, F_EDICT),
    EDICTFIELD(inventory[0], F_EDICT), EDICTFIELD(inventory[1], F_EDICT), EDICTFIELD(inventory[2], F_EDICT),
    EDICTFIELD(inventory[3], F_EDICT), EDICTFIELD(inventory[4], F_EDICT), EDICTFIELD(inventory[5], F_EDICT),
    EDICTFIELD(cargo.units[0], F_EDICT), EDICTFIELD(cargo.units[1], F_EDICT), EDICTFIELD(cargo.units[2], F_EDICT),
    EDICTFIELD(cargo.units[3], F_EDICT), EDICTFIELD(cargo.units[4], F_EDICT), EDICTFIELD(cargo.units[5], F_EDICT),
    EDICTFIELD(cargo.units[6], F_EDICT), EDICTFIELD(cargo.units[7], F_EDICT), EDICTFIELD(item.carrier, F_EDICT),
    EDICTFIELD(ground_next, F_EDICT), EDICTFIELD(movement.attackmove_waypoint, F_EDICT),
    EDICTFIELD(movement.patrol_a, F_EDICT), EDICTFIELD(movement.patrol_b, F_EDICT),
    EDICTFIELD(movement.patrol_target, F_EDICT), EDICTFIELD(goalentity, F_EDICT), EDICTFIELD(combatentity, F_EDICT),
    EDICTFIELD(secondarygoal, F_EDICT), EDICTFIELD(owner, F_EDICT), EDICTFIELD(build, F_EDICT),
    { NULL, 0, 0, 0 }
};
