...
static int32_t QuestPeonStageTriggerOrdinal(LPTRIGGER trigger) {
...
static void TutorialFlowDebugLogRegistration(LPTRIGGER trigger, EVENTTYPE type,
                                             LPEDICT subject, cstring_t registration) {
...
uint32_t CreateTrigger(LPJASS j) {
...
uint32_t DestroyTrigger(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    return 0;
}
uint32_t ResetTrigger(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    return 0;
}
uint32_t EnableTrigger(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t DisableTrigger(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    if (QuestPeonStageDebugEnabled() && QuestPeonStageTrigger(whichTrigger)) {
        LPCJASSCONTEXT ctx = jass_getcontext(j);
        cstring_t caller = ctx ? jass_functionname(ctx->func) : NULL;
...
uint32_t IsTriggerEnabled(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerWaitOnSleeps(LPJASS j) {
...
uint32_t IsTriggerWaitOnSleeps(LPJASS j) {
...
uint32_t GetTriggeringTrigger(LPJASS j) {
    LPCJASSCONTEXT ctx = jass_getcontext(j);
...
uint32_t GetTriggerEventId(LPJASS j) {
    return jass_pushnullhandle(j, "eventid");
}
uint32_t GetTriggerEvalCount(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    return jass_pushinteger(j, 0);
}
uint32_t GetTriggerExecCount(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    return jass_pushinteger(j, 0);
}
...
uint32_t TriggerRegisterVariableEvent(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerRegisterTimerEvent(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerRegisterTimerExpireEvent(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerRegisterGameStateEvent(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t GetTriggeringRegion(LPJASS j) {
...
uint32_t TriggerRegisterLeaveRegion(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    handle_t region = jass_checkhandle(j, 2, "region");
    LPREGION whichRegion = G_RegionFromHandle(region);
    LPCJASSFUNC filter = jass_checkhandle(j, 3, "boolexpr");
...
uint32_t TriggerRegisterTrackableHitEvent(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    //handle_t t = jass_checkhandle(j, 2, "trackable");
    return jass_pushnullhandle(j, "event");
}
uint32_t TriggerRegisterTrackableTrackEvent(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    //handle_t t = jass_checkhandle(j, 2, "trackable");
    return jass_pushnullhandle(j, "event");
}
uint32_t TriggerRegisterPlayerEvent(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t GetTriggerPlayer(LPJASS j) {
    return jass_pushlighthandle(j, jass_getcontext(j)->playerState, "player");
}
uint32_t TriggerRegisterPlayerUnitEvent(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    LPPLAYER whichPlayer = jass_checkhandle(j, 2, "player");
...
uint32_t TriggerRegisterPlayerAllianceChange(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    //LPPLAYER whichPlayer = jass_checkhandle(j, 2, "player");
    //handle_t whichAlliance = jass_checkhandle(j, 3, "alliancetype");
    return jass_pushnullhandle(j, "event");
}
uint32_t TriggerRegisterPlayerStateEvent(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    //LPPLAYER whichPlayer = jass_checkhandle(j, 2, "player");
...
uint32_t TriggerRegisterPlayerChatEvent(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    //LPPLAYER whichPlayer = jass_checkhandle(j, 2, "player");
...
uint32_t TriggerRegisterDeathEvent(LPJASS j) {
...
uint32_t TriggerRegisterUnitStateEvent(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerRegisterUnitEvent(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    LPEDICT whichUnit = jass_checkhandle(j, 2, "unit");
...
uint32_t TriggerRegisterFilterUnitEvent(LPJASS j) {
    //LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerRegisterUnitInRange(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    LPEDICT whichUnit = jass_checkhandle(j, 2, "unit");
...
uint32_t TriggerAddCondition(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerRemoveCondition(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerClearConditions(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    DELETE_LIST(TRIGGERCONDITION, whichTrigger->conditions, gi.MemFree);
    return 0;
}
uint32_t TriggerAddAction(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerRemoveAction(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerClearActions(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
...
uint32_t TriggerSleepAction(LPJASS j) {
...
uint32_t TriggerWaitForSound(LPJASS j) {
...
uint32_t TriggerEvaluate(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    LPCJASSCONTEXT context = jass_getcontext(j);
...
uint32_t TriggerExecute(LPJASS j) {
    LPTRIGGER whichTrigger = jass_checkhandle(j, 1, "trigger");
    LPCJASSCONTEXT context = jass_getcontext(j);
    if (QuestPeonStageDebugEnabled() && QuestPeonStageTrigger(whichTrigger)) {
        LPCJASSCONTEXT ctx = jass_getcontext(j);
...
uint32_t TriggerExecuteWait(LPJASS j) {
...
uint32_t GetTriggerUnit(LPJASS j) {
    return jass_pushlighthandle(j, jass_getcontext(j)->unit, "unit");
}
uint32_t GetTriggerWidget(LPJASS j) {
...
uint32_t GetTriggerDestructable(LPJASS j) {
