...
static COLOR32 leaderboard_color(LONG r, LONG g, LONG b, LONG a) {
    return MAKE(COLOR32, (BYTE)MAX(0, MIN(255, r)), (BYTE)MAX(0, MIN(255, g)),
                (BYTE)MAX(0, MIN(255, b)), (BYTE)MAX(0, MIN(255, a)));
}
static struct gleaderboarditem_s *leaderboard_item(LPLEADERBOARD board, LONG index) {
    return board && board->inuse && index >= 0 && (DWORD)index < board->item_count
...
DWORD LeaderboardAddItem(LPJASS j) {
    LPLEADERBOARD b = jass_checkhandle(j, 1, "leaderboard"); LPCSTR label = jass_checkstring(j, 2);
    LONG value = jass_checkinteger(j, 3); LPPLAYER p = jass_checkhandle(j, 4, "player");
    ...
    item->player = p ? (LONG)PLAYER_NUM(p) : -1; item->show_label = item->show_value = item->show_icon = true;
...
DWORD LeaderboardSortItemsByValue(LPJASS j) { leaderboard_sort(jass_checkhandle(j, 1, "leaderboard"), 0, jass_checkboolean(j, 2)); return 0; }
DWORD LeaderboardSortItemsByPlayer(LPJASS j) { leaderboard_sort(jass_checkhandle(j, 1, "leaderboard"), 1, jass_checkboolean(j, 2)); return 0; }
DWORD LeaderboardSortItemsByLabel(LPJASS j) { leaderboard_sort(jass_checkhandle(j, 1, "leaderboard"), 2, jass_checkboolean(j, 2)); return 0; }
...
DWORD LeaderboardSetLabelColor(LPJASS j) { LPLEADERBOARD b=jass_checkhandle(j,1,"leaderboard"); LONG r=jass_checkinteger(j,2),g=jass_checkinteger(j,3),bl=jass_checkinteger(j,4),a=jass_checkinteger(j,5); if(b&&b->inuse){b->label_color=leaderboard_color(r,g,bl,a);b->label_color_set=true;G_MarkLeaderboardDirty(b);} return 0; }
DWORD LeaderboardSetValueColor(LPJASS j) { LPLEADERBOARD b=jass_checkhandle(j,1,"leaderboard"); LONG r=jass_checkinteger(j,2),g=jass_checkinteger(j,3),bl=jass_checkinteger(j,4),a=jass_checkinteger(j,5); if(b&&b->inuse){b->value_color=leaderboard_color(r,g,bl,a);b->value_color_set=true;G_MarkLeaderboardDirty(b);} return 0; }
...
DWORD LeaderboardSetItemValue(LPJASS j) { LPLEADERBOARD b=jass_checkhandle(j,1,"leaderboard"); struct gleaderboarditem_s *i=leaderboard_item(b,jass_checkinteger(j,2)); LONG v=jass_checkinteger(j,3); if(i){i->value=v;G_MarkLeaderboardDirty(b);} return 0; }
DWORD LeaderboardSetItemLabel(LPJASS j) { LPLEADERBOARD b=jass_checkhandle(j,1,"leaderboard"); struct gleaderboarditem_s *i=leaderboard_item(b,jass_checkinteger(j,2)); LPCSTR v=jass_checkstring(j,3); if(i){strlcpy(i->label,G_LevelString(v?v:""),sizeof(i->label));G_MarkLeaderboardDirty(b);} return 0; }
DWORD LeaderboardSetItemStyle(LPJASS j) { LPLEADERBOARD b=jass_checkhandle(j,1,"leaderboard"); struct gleaderboarditem_s *i=leaderboard_item(b,jass_checkinteger(j,2)); BOOL l=jass_checkboolean(j,3),v=jass_checkboolean(j,4),icon=jass_checkboolean(j,5); if(i){i->show_label=l;i->show_value=v;i->show_icon=icon;G_MarkLeaderboardDirty(b);} return 0; }
DWORD LeaderboardSetItemLabelColor(LPJASS j) { LPLEADERBOARD b=jass_checkhandle(j,1,"leaderboard"); struct gleaderboarditem_s *i=leaderboard_item(b,jass_checkinteger(j,2)); LONG r=jass_checkinteger(j,3),g=jass_checkinteger(j,4),bl=jass_checkinteger(j,5),a=jass_checkinteger(j,6); if(i){i->label_color=leaderboard_color(r,g,bl,a);i->label_color_set=true;G_MarkLeaderboardDirty(b);} return 0; }
DWORD LeaderboardSetItemValueColor(LPJASS j) { LPLEADERBOARD b=jass_checkhandle(j,1,"leaderboard"); struct gleaderboarditem_s *i=leaderboard_item(b,jass_checkinteger(j,2)); LONG r=jass_checkinteger(j,3),g=jass_checkinteger(j,4),bl=jass_checkinteger(j,5),a=jass_checkinteger(j,6); if(i){i->value_color=leaderboard_color(r,g,bl,a);i->value_color_set=true;G_MarkLeaderboardDirty(b);} return 0; }
