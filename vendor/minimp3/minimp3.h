...
/* Zero-pad short Huffman caches without forming pointers past the main-data buffer. */
#define CHECK_BITS    while (bs_sh >= 0) { if (bs_next_byte < bs_limit_bytes) bs_cache |= (uint32_t)bs->buf[bs_next_byte] << bs_sh; bs_next_byte++; bs_sh -= 8; }
#define BSPOS         (bs_next_byte*8 - 24 + bs_sh)

    float one = 0.0f;
    int ireg = 0, big_val_cnt = gr_info->big_values;
    const uint8_t *sfb = gr_info->sfbtab;
    int bs_next_byte = bs->pos/8, bs_limit_bytes = bs->limit/8;
    uint32_t bs_cache = 0;
    int pairs_to_decode, np, bs_sh = (bs->pos & 7) - 8;
    for (int i = 0; i < 4; i++, bs_next_byte++)
        bs_cache = (bs_cache << 8) | (bs_next_byte < bs_limit_bytes ? bs->buf[bs_next_byte] : 0);
    bs_cache <<= (bs->pos & 7);
