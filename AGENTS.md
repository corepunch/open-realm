...
- **Use the engine screenshot command, not desktop capture.** `screenshot` captures the GL drawable to `screenshots/shotNNNN.png`; `screenshot N` captures the Nth fully rendered frame after the command executes. From the command line, place it after the scene/map selector and keep `com_frame_limit` larger than the delay: `build/bin/openwow -data data/world-of-warcraft +map playercreate +screenshot 10 +com_frame_limit 20`. Use `+screenshot 1` for the first rendered frame or omit the number for the next rendered frame.
