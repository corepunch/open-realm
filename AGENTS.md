...
- Keep runtime structs concise. Group related fields; use anonymous structs for repeated shapes; prefer `uint32_t flags` over many standalone `bool` fields.
- Declare a pointer + element-count pair as one unit with `ARRAY(type, name)` (defines `type *name; uint32_t name##_count;`). Access the count via `ARRAY_COUNT(name)`, test emptiness with `IS_ARRAY_EMPTY(name)` (checks both pointer and count), and iterate with `FOR_EACH_ARRAY(type, it, name)` — or `FOR_LOOP(i, ARRAY_COUNT(name))` when the index is needed — never read or write `name##_count` directly.
...
- Name struct typedefs in lower camel case with a `_t` suffix, such as `portraitFog_t`.
- Write struct pointers directly as `portraitFog_t *value` or `portraitFog_t const *value`. Keep `const` after the type name and the `*` beside the variable name.
- Use `<stdint.h>` widths and `<stdbool.h>` booleans. Do not add WinAPI-style integer or `LP`/`LPC` pointer aliases.
- `string_t`, `cstring_t`, and `handle_t` are the existing named aliases for strings and opaque handles.
