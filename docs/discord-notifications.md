...
[Discord Notify](../.github/workflows/discord-notify.yml) posts one concise webhook embed when a
pull request is opened (`pull_request_target`, types `opened` and `ready_for_review`).
Draft PRs post only when marked ready for review; PRs opened by bot accounts never post.
Reopened, closed, merged, issue, and release events post nothing. This workflow is the
...
(`name` = PR author login, `url` = author profile URL, `icon_url` = author avatar URL). The clickable title holds
...
PR variables (including `PR_AVATAR` and `PR_AUTHOR_URL`) and prepend `curl() { cat; }`. Inspect the resulting JSON:
one embed with the expected `author.name`, `author.url`, `author.icon_url`, `title`, `url`, and `color`.
