...
| `Adro` | Drop | Partial | Unload All now ejects the first occupant immediately and then one occupant per Cargo Hold `Dur` until empty, matching Warsmash's repeated cargo behavior while reusing shared unstuck/unhide/unpause removal. The current selected point is still not used for transport movement/placement, so exact authored point semantics remain. |
