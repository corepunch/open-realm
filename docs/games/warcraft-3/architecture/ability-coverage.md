...
| `Abli` | Blight | Partial | Passive Warsmash-style `DataA` create/remove, `DataB` expansion, `Area` cap, and `Duration` cadence mutate the authoritative WC3 game-owned Blight field. Placement, `uhrt=blight`, JASS queries/mutations, and save/load consume the same state. Runtime terrain rendering/client preview sync and Blighted trees remain. |
