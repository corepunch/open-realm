...
| `AHwe`, `AOsf` | `s_summon.c` | Partial no-target summon; DataA count, level unit id, duration, ability-owned summon identity, TargetArt, timed life, and unit/player summon events are implemented. Water Elemental also uses its authored BuffID for summon status/presentation, forward Area placement, caster facing, collision-safe separation, and relinks displaced summons. `BTLF` remains the authoritative timed-life clock and is protected from Dispel/Purge so surviving summons cannot become permanent. |
...
| `AHbz`, `AUcs`, `ANcl` | `s_area_spell.c` | Partial point/channel spell family; Blizzard uses authored target masks, DataA/B/C/D/F wave fields, structure scaling, shard art/sound resolved through its level EfctID object (`AbilitySounds.slk` for `Effectsound`), aggregate wave caps, authored first-wave/interval timing, Warsmash-style 0.8-second shard-to-impact phases, save/load-safe channel state, and cancellation. Carrion Swarm applies a simple point blast; Channel opens cancel mode. |
| `AHmt` | `s_requested_abilities.c` | Partial Mass Teleport; DataB channel delay, target-incarnation cancellation, destination pause/restore, source/destination/caster/special art, caster-inclusive DataA cap, caster-player mobile-unit filtering, DataC clustering, collision-safe relocation, broad-phase relinking/FOW invalidation, and save/load continuation are implemented. |
...
| `AHwe` | Summon Water Elemental | Partial | No-target summon reads DataA count and level UnitID, marks each result as an ability-created summon, keeps `BTLF` as the timed-life clock, adds the authored BuffID for summon status/presentation, publishes summon events, and plays TargetArt. Forward-facing Area placement, caster facing, collision-safe multi-summon separation, and server relinking are implemented; Dispel/Purge preserve `BTLF` while still applying summoned-unit damage. Remaining work is finer order/presentation polish. |
| `AHbz` | Blizzard | Partial | Point targeting, current-rank authored target masks, DataA wave count, DataB damage, DataC shard count, level EfctID-driven shard EffectArt, DataD structure scaling, DataF aggregate wave cap, authored first-wave/interval timing, 0.8-second shard-to-damage sub-waves, channel cancellation, and save/load continuation exist. Per-shard `Effectsound` now resolves through `AbilitySounds.slk`; richer SPELLS/COLD damage typing and full pitch/variant audio presentation still need retail parity. |
| `AHab` | Brilliance Aura | Mostly implemented | Passive DataA mana regeneration uses the shared two-second aura cache, authored Area/`targs`, alias-specific object data, and strongest-copy semantics. DataB now selects flat MP/sec versus percentage of intrinsic mana regeneration; flat and percentage variants use separate strongest-copy families. Sparse rows without `targs` retain the legacy friendly fallback. Broader mechanical/organic target-class normalization remains shared target-system work. |
| `AHmt` | Mass Teleport | Partial | Unit-target channel uses DataB delay, validates the current-rank authored target mask and target identity through completion, pauses/restores the destination, cancels on interrupted channels or lost targets, limits the caster plus nearby owned mobile units by DataA, honors DataC clustering, emits authored effects, relinks moved units in the server broad phase, and survives save/load. Broader target-mask vocabulary and retail placement/order polish remain. |
