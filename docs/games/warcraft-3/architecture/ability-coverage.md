...
| `Awrp` | `s_waygate.c`, `api/api_unit.h` | Partial; `CAbilityWarp` owns explicit Smart traversal, authored rectangular entry dimensions, interruption cleanup, and JASS state. `m_unit.c` only supplies generic target-order/order-accepted hooks. Automatic portal-aware point routing remains. See [Way Gates](../way-gates.md). |
...
| `AIpm` | `s_wards.c`, `g_items.c`, `s_spell.c` | Point-target mine placement through authored UnitID; successful targeted item completion consumes exactly one charge while invalid/cancelled targets do not. See [Land Mines](../land-mines.md). |
| `Amin` | `s_wards.c` | Innate mine activation/invisibility/proximity behavior; DataA arm delay, DataB invisibility transition, Cast Range trigger radius, walk-over collision, True Sight integration, rooted-Ancient exclusion/mobile-uprooted exception, and normal self-death dispatch are implemented. See [Land Mines](../land-mines.md). |
| `Adda`, `Amnx` | `s_creep_abilities.c` | Two-ring authored death AOE with target-mask filtering, Duration-delayed victim enumeration at the saved death position, and authored tree/wall/debris/bridge/decoration destructible damage. See [Land Mines](../land-mines.md). |
...
| `Aens`, `ANen`, `ACen` | `s_campaign_abilities.c` | `CAbilityEnsnare` + `AB_UPDATE`. TFT `Bena`/`Beng` by air/ground, ROC empty BuffID → `Bens`. `S_UnitIsEnsnared` locks `order_move` with `BEer`. Flyers clear `AI_FLYING` immediately; DataA/B gradual land/rise; DataC melee attack range via `S_EnsnareMeleeRange`. Flight/phase ownership lives in the ability via `A_STATUS_REFRESH`/`A_STATUS_REMOVE`; `m_unit.c` keeps storage/iteration/timing/dispatch. |
...
| `AIpm` | Item Place Goblin Land Mine | Partial | Authored UnitID point summon, ownership/summon lifecycle, and post-success charged-item consumption are wired. Placement/presentation edge cases beyond the shared point-spell validator remain. See [Land Mines](../land-mines.md). |
| `Amin` | Mine - exploding | Partial | DataA arming, DataB invisibility transition, Cast Range enemy-ground trigger, walk-over collision, True Sight integration, rooted-Ancient/mobile-uprooted distinction, normal self-death, `Death Spell`, and renderer `SPN` child-model presentation are wired. Broader unusual target edge cases remain. See [Land Mines](../land-mines.md). |
| `Amnx` / `Adda` | AOE Damage Upon Death | Partial | DataA/B full ring, DataC/D partial ring, authored unit target masks, Duration-delayed resolution at the death position, and authored destructible target classes are wired. Mine explosion presentation is model-driven through `Death Spell`/`SPN`; broader alliance/target-mask edge cases remain. See [Land Mines](../land-mines.md). |
...
| `Awrp` | Way Gate | Partial | `Awrp`-derived aliases read authored DataA/DataB as the rectangular entry area; `CAbilityWarp` owns target-order validation, approach/cleanup, and traversal. All five `Waygate*` JASS natives, active `alternate` presentation, static-pathing entrance selection, unstuck relocation, generation guarding, Shift continuation, and save/load are implemented. Ordinary Move/Attack-Move pathfinding does not yet discover portal routes automatically. See [Way Gates](../way-gates.md). |
...
| `AIso` | Item Soul Theft / Soul Trap | Partial | Retail `CAbilitySoulTrap` uses the shared unit-target spell pipeline and authored target masks; it preserves the originating item through approach, publishes successful use-item events, traps the same target edict without death, binds the filled `soul`, and reveals the carrier to the trapped unit's owner. Custom target-mask parity remains. |
| `Asou` | Item Soul Possession | Partial | `CAbilitySoulTrapped` owns the persistent carrier/target relationship, carrier-death release at the death position, carrier-removal cleanup, and target-removal item cleanup. Unusual custom targets and subsystem-specific timer/building behavior remain unverified. |
...
| `Acar` | Cargo Hold | Partial | Per-holder capacity, real-unit cargo storage, hidden+paused occupants, single-passenger cargo-slot unload, timed Unload All spacing from the hold's authored `Dur`, death ejection, loaded-state JASS queries, selected-unit cargo slots, and shared world-hover segmented occupancy presentation exist. Needs broader mobile walk-into-range compatibility and exact retail segment styling verification. |
| `Amel`, `Amed`, `Amtc`/`Sch2` | Meat Wagon corpse cargo | Partial | Data-driven capacity, real corpse-edict storage, decay suspension, holder-relative corpse position, corpse-only `Amtc` gating that rejects living-unit Load/Smart boarding, Get Corpse approach plus acquisition-range autocast, authored target filtering without a hard-coded corpse-owner restriction, Drop All, death ejection, Exhume integration, stored-corpse Raise Dead/Cannibalize consumption, and segmented hover occupancy exist. Exact spill/unload placement/facing and flesh-phase unload semantics remain. |
...
| `Adro` | Drop | Partial | Unload All now ejects the first occupant immediately and then one occupant per Cargo Hold `Dur` until empty, using an active ability move so Stop/Move cancel it and pause/stun suspend it; save/load resumes the remaining sequence. See [shared transport unloading](../orc-burrows.md#shared-transport-unloading). The current selected point is still not used for transport movement/placement, so exact authored point semantics remain. |
| `Adri` | Drop Instant | Partial | No-target Unload All Instant now ejects every occupant immediately through the shared cargo-removal path. Exact reference presentation/sound semantics remain. |
