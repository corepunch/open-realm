...
Pending construction Birth sites reserve placement and show the building model, but remain walk-through: they do not enter static routing until `G_CompleteConstruction()` changes the structure to its completed state. A worker already in the footprint is displaced at construction start, while a worker that reaches the footprint during Birth may continue through it. The live-entity placement check still treats an existing construction as a hard building blocker, so a later worker can show a competing Birth preview but fails its arrival-time placement validation if the first structure has already spawned. Static building footprints are rebuilt after completion and building death. `common/routing.c` excludes dead `SVF_MONSTER` entities from static footprint baking even when their authored `pathtex` remains attached for presentation/save state; dead destructables retain their separate death-path-texture behavior. This prevents cancelled or destroyed buildings from leaving invisible route blockers.
...
The building's birth animation is held until construction completes. Human construction is selected from the builder's `Arep` capability, not from `UnitData.race`. Its authored pathing footprint is used directly to relocate the primary worker, but is not baked into static routing until completion; ordinary Repair orders approach their target through movement/pathfinding and never teleport to the structure. Selection radius is not a substitute for pathing geometry. This preserves the retail behavior where other workers can walk through a Birth site while preventing the Lumber Mill case where the old `SP_FindEmptySpaceAround()` placed the worker inside cells that became blocked after construction completed. The primary worker then contributes `1.0` construction time. If that worker stops, dies, or receives another behavior, Repair teardown clears the primary-builder association and no autonomous timer advances the paused structure. A later Human Repair worker can become the new primary worker.
