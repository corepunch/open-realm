...
- live unit-circle occupancy, excluding the builder and non-blocking pending Birth previews;
...
Placement is checked once when the player confirms the ghost and again when the worker reaches the site. An accepted worker order creates a server-side Birth preview at the snapped point. The preview has the building model and Birth animation but zero gameplay collision; it does not bake static pathing, so units can walk through it and multiple workers can reserve the same visual point. Resources are charged only after the arrival-time validation passes. Construction removes the winning worker's preview and spawns at the stored snapped waypoint, not at the worker's current position. If another construction has won the point first, a later worker fails the arrival check and receives `Unable to build there.`; its preview is removed and no structure is spawned. Generic placement rejection uses the plain UI message `Unable to build there.`; the Gold Mine exclusion result uses `Unable to build so close to the gold mine.` and resolves its UI sound through the Warcraft `Tooclosetomine` skin key. Requirement/resource failures remain separate command-state messages. `G_LevelString()` resolves only exact `TRIGSTR_<id>` tokens. Previously it parsed arbitrary UI text as trigger-string ID 0, so Human02's WTS entry 0 (the map name) replaced ordinary errors with `Human02`.
