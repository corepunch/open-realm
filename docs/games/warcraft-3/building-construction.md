...
This same data-driven path covers defensive structure upgrades such as Ziggurat -> Spirit Tower when the mounted Warcraft data exposes that `uupt` target and its prerequisites. The source building's Food Made contribution remains active during the upgrade; completion replaces it with the target unit's authored Food Made value. Ordinary attacks/idle acquisition are suppressed while `G_BuildingUpgradeActive()` is true, and the completed target receives its authored weapon/target-mask state through the normal unit transform/spawn path. Dynamic Ziggurat/Spirit Tower Blight growth is a separate unresolved terrain mechanic; see [Race Mechanics](race-mechanics.md).
...
- live unit-circle occupancy, excluding the builder and non-blocking Construction Site Indicators;
...
Placement is checked once when the player confirms the ghost and again when the worker reaches the site. An accepted worker order creates a server-side Construction Site Indicator at the snapped point in both RoC and TFT modes. The indicator uses the normal building `Stand` presentation, freezes its first frame, and applies white vertex colour with alpha 128; it does not use the building's `Birth` animation. It is private order presentation: `s.player` identifies the issuing player and `SVF_OWNER_ONLY` keeps it out of every ally/opponent snapshot, while the real construction entity uses normal world visibility once the worker arrives. The indicator has zero gameplay collision and does not bake static pathing, so units can walk through it and multiple workers can reserve the same visual point. Resources are charged only after the arrival-time validation passes. Construction removes the winning worker's indicator and spawns the real building at the stored snapped waypoint, not at the worker's current position. The real building then owns the normal `Birth`/construction presentation. If another construction has won the point first, a later worker fails the arrival check and receives `Unable to build there.`; its indicator is removed and no structure is spawned. Generic placement rejection uses the plain UI message `Unable to build there.`; the Gold Mine exclusion result uses `Unable to build so close to the gold mine.` and resolves its UI sound through the Warcraft `Tooclosetomine` skin key. Requirement/resource failures remain separate command-state messages. `G_LevelString()` resolves only exact `TRIGSTR_<id>` tokens. Previously it parsed arbitrary UI text as trigger-string ID 0, so Human02's WTS entry 0 (the map name) replaced ordinary errors with `Human02`.
...
Gold-return town-hall previews add one whole-footprint rule on top of those per-cell tests. Unit ability data marks the cursor with `EF_RESOURCE_GOLD_RETURN`, while live `Agld`-class mine snapshots carry `EF_RESOURCE_GOLD_SOURCE`. If the snapped cursor centre is less than `RESOURCE_GOLD_SOURCE_MIN_DISTANCE` (512 world units) from a visible live mine snapshot, every placement-grid cell is drawn red. The server repeats the same centre-distance rule from authoritative entity data, so cursor presentation cannot make an otherwise forbidden placement legal; a mine unavailable to the client's current snapshot can therefore still be rejected authoritatively when clicked.
...
Construction Site Indicators reserve placement visually with a translucent copy of the normal building model in both RoC and TFT, but remain walk-through until the worker arrives. They are owner-only snapshot entities, so other players do not learn a pending build from the marker before the real construction begins. Friendly mobile units are displaced immediately when the indicator is placed; if one returns before arrival, the same displacement is repeated when the real construction entity is spawned. Retail clears the four pathing-cell approach lane around the reserved footprint, not only units already overlapping the footprint; this lets a worker whose existing route is cut by construction continue to its later destination. The indicator remains excluded from static routing, while the real unfinished building is baked as a route obstacle before Repair/build work begins. A worker that reaches the footprint during the indicator window may continue through it, but cannot walk through the construction entity afterward. The live-entity placement check still treats an existing construction as a hard building blocker, so a later worker can show a competing Construction Site Indicator but fails its arrival-time placement validation if the first structure has already spawned. Static building footprints are rebuilt after construction start, completion, and building death. `common/routing.c` excludes dead `SVF_MONSTER` entities from static footprint baking even when their authored `pathtex` remains attached for presentation/save state; dead destructables retain their separate death-path-texture behavior. This prevents cancelled or destroyed buildings from leaving invisible route blockers.

Build the WC3 game with `WC3_DEBUG_BUILD=1` to enable bounded `WC3_BUILD` diagnostics for accepted/rejected orders, region entry, preview lifecycle, arrival validation, displacement, construction start/stop/cancel, and Repair teardown. The trace is compile-time gated and is off in normal builds.
...
The building owns temporary-worker state (`construction.worker` plus `spawn_time`) separately from Human `construction.primary_builder`. Completion, cancellation, combat death, and direct `G_FreeEdict()` all use the same worker-release path. Human Repair rejects active autonomous construction, so it cannot become a second construction clock. The per-player `instant build`/`warpten` cheat is checked only by this shared construction state before the normal Human paused-strategy gate, so every started standard-race structure completes through `G_CompleteConstruction()` on its next entity frame regardless of worker update order while preserving the ordinary worker-release, event, food, sound, and UI lifecycle. The older self-linked legacy Repair construction path remains for custom/unknown workers. Its initial zero health is a construction state: the worker may repair it only while the building self-link is active, then it completes through `G_CompleteConstruction()`.
...
The pending Construction Site Indicator holds the first frame of the building's normal `Stand` animation at 50% alpha; it is a marker, not construction progress. Once construction starts, the indicator is removed and the real building advances `Birth` from authoritative construction progress. Human construction is selected from the builder's `Arep` capability, not from `UnitData.race`. Its authored pathing footprint is used directly to relocate occupants before the route obstacle is baked; ordinary Repair orders approach their target through movement/pathfinding and never teleport to the structure. Selection radius is not a substitute for pathing geometry. This preserves the retail behavior where other workers can walk through a pending Construction Site Indicator but not through the real construction site. The primary worker then contributes `1.0` construction time. If that worker stops, dies, or receives another behavior, Repair teardown clears the primary-builder association and no autonomous timer advances the paused structure. A later Human Repair worker can become the new primary worker.
...
For completed Repair targets (structures and authored mechanical units), the current implementation uses:
...
Gold and lumber use per-worker floating-point accumulators; only whole accumulated resources are deducted. Affordability is checked before the corresponding HP/progress update. The `wc3_build_all` override continues to suppress incremental **power-build** cost only; it does not make ordinary completed-target Repair free.

Repair range comes from ability `Rng`. Buildings retain footprint-aware contact through `CM_DistanceToPathingFootprint()` when authored pathing exists, with the legacy collision-circle distance as a no-footprint fallback. Repairable mobile mechanical units instead remain a live entity `goalentity`, use collision-edge range, and can therefore be followed while moving. Repair `DataE` is added to `Rng` only when the target's authored movement type is `float`, matching Warsmash's naval-range rule. When in range, the worker uses `stand work`. Leaving Repair through another order or death releases its Repair state and any primary-builder ownership.

Smart/right-click tries Repair on an otherwise non-enemy target before falling back to Move. A full-health target declines Smart Repair silently. Explicit Repair reports `Target is not damaged.` for a completed full-health target and rejects standard Repair on construction.

Repair also participates in the generic autocast hooks documented in [autocast.md](autocast.md). Right-clicking the Repair command button sends the secondary `autocast <rawcode>` command while left click remains ordinary target selection. `Arep`, `Aren`, and `Arst` share the persisted `AI_AUTOCAST_REPAIR` toggle. During an automatic acquisition opportunity, Repair scans the worker's `runtime.acquisition_range`, chooses the **nearest** target accepted by the same Repair validation used by an explicit order, then submits the normal `repair` entity order. The existing Repair behavior therefore remains authoritative for movement into `Rng`, building-footprint or mobile-unit collision contact, costs, work animation, construction contribution, interruption, and completion. Completing one target does not disable Auto Repair; a later idle/default stand acquisition pass may choose another damaged structure or mechanical unit. Repair cancellation clears `goalentity` only when it still names the Repair-owned target: Stop/stand therefore retires the old target, while a replacement Move/Attack goal installed before the behavior switch is preserved.
...
Repair now evaluates the high-confidence subset of WC3 `Targets Allowed` needed by the stock Repair family instead of hard-coding owned buildings. Sparse/custom rows with no `targs` retain the legacy building-only fallback. With authored targets, structures require `structure`; non-buildings require the `mechanical` unit classification **and** the `mechanical` target token; mobile units honor authored `ground`/`air`; `ancient` / `nonancient` constrain the unit classification; and `friend` / `allies` permits the owner or passive allies through the normal alliance helper. Allied completed targets can be repaired, but Human power building remains owner-only and an allied Repair attempt cannot take over or clear another player's `primary_builder`. Repair deliberately does **not** route this through `S_SpellAllowsTarget()` because that helper models a smaller spell-oriented subset and cannot express Repair's overlapping physical/classification groups safely. Destructibles and the rest of the general target-mask surface remain separate gaps.
...
- Orc worker-inside, Night Elf worker/Ancient consumption, and Undead summon/release construction strategies are implemented; Haunted/Entangled Mine overlays, authoritative Blight placement, and Blight presentation/client sync are implemented, while Wisp lumber and full Root/Uproot remain tracked in [Race Mechanics](race-mechanics.md) and [Blight](blight.md);
