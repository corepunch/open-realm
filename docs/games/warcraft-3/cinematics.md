...
- `SetSoundVolume`, `SetSoundPosition`, and `AttachSoundToUnit` now feed the existing one-shot sound packet. Campaign MP3 dialogue uses the generic one-shot decoder; stop/fade, volume groups, and thematic music remain separate audio gaps.
