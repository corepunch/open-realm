...
- the WC3 terrain renderer compares synchronized corner state with the W3E-authored corner state, leaving authored map Blight to the static terrain baseline and rendering only runtime-added corners; those additions use the atlas's authored alpha edge masks instead of opaque square decals, are cached per 128-unit tile, and rebuild only tiles intersecting changed network rows; it resolves the W3E tileset through `UI\\WorldEditData.txt` and uses the corresponding `TerrainArt\\Blight\\*_Blight.blp` atlas with authored ground-variation UVs;
