...
Retail 1.30+ `ConsoleUI.fdf` uses symbolic `ConsoleTexture05` and `ConsoleTexture06` for the left/right widescreen console extensions, anchored 0.256 outside the 4:3 root on each side; classic archives author no such frames. The server resolves symbolic keys through `UI_ThemeImagePath` and the recipient's `war3skins.txt` race category before registering concrete `CS_IMAGES` paths. Some classic/pre-widescreen skin tables contain the extension art in the archives but omit one or both symbolic fields. In that case the skin lookup lacks a value and the renderer otherwise searches for a literal file named `ConsoleTexture06`, producing the 16x16 placeholder.

When one console extension key remains unresolved, `UI_ThemeImagePath()` derives its path from the resolved sibling (`...05[.blp]` ↔ `...06[.blp]`). This keeps the active race/custom skin authoritative instead of hard-coding a race path. Runtime confirmation on a 2880x1620 Human campaign showed the left extension resolving to `HumanUITile05` while the missing right extension was laid out correctly but retained literal `ConsoleTexture06`; this is a media-resolution failure, not an anchor failure.

**Class-gated, deferred registration.** These two keys are the exception to parse-time registration. `UI_LoadTexture` gives them a handle at or above `HUD_DEFERRED_IMAGE_BASE` (`hud.deferred_key[]`) instead of calling `gi.ImageIndex`, `UI_LoadHudConsole` collects the frames into `hud.console_wide[]`, and `UI_WriteConsoleBackdrop` hides them for recipients whose `client_s.canvas` is not `UI_CANVAS_WIDE`. `UI_LiveImage` resolves a deferred handle for the recipient's race and registers it the first time a wide client's console is written, so sessions on 4:3 windows never carry the tiles in `CS_IMAGES`. `UI_ImageKey` returns the symbolic key for either handle range. Once registered the slots follow the normal per-level lifetime above. See [ui-canvas.md](../../architecture/ui-canvas.md) for the client side (window class, settle timer, `ui_canvas` command).
