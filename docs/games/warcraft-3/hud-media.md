...
`ConsoleUI.fdf` uses symbolic `ConsoleTexture05` and `ConsoleTexture06` for the left/right widescreen console extensions. The server resolves symbolic keys through `UI_ThemeImagePath` and the recipient's `war3skins.txt` race category before registering concrete `CS_IMAGES` paths. Some classic/pre-widescreen skin tables contain the extension art in the archives but omit one or both symbolic fields. In that case the skin lookup lacks a value and the renderer otherwise searches for a literal file named `ConsoleTexture06`, producing the 16x16 placeholder.
