...
Version 37 adds the per-unit `corpse_unraisable` and `corpse_no_decay` lifecycle flags. Death Pact sacrifices and consumed Animated Dead corpses therefore cannot become raisable again after save/load, and no-decay victims still disappear after their death animation instead of entering the normal 90-second corpse window. The exact-version guard rejects version 36 and earlier saves.
