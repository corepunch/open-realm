...
This document records the Human07 (`Maps\\Campaign\\Human07.w3m`) lifecycle findings: the
...
| The campaign script could not launch its authored assault | OpenRealm did not expose the `SuicidePlayer` AI native used by the campaign AI path. | Implement and register `G_BotSuicidePlayer`: validate the AI player, captain, and `check_full`; issue attack orders against valid units owned by the target player, falling back to the staged point when no target is visible; launch live captain members and update captain state. |
