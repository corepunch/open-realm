...
/* Find the closest standable point inside a world-space rectangle using only
 * immutable/static pathing. Temporary unit occupancy is intentionally excluded:
 * callers use this to choose a durable interaction destination, while ordinary
 * move-time collision/local avoidance resolves transient crowds. */
BOOL CM_ClosestStaticPathablePointInRectForRadiusFlags(LPCVECTOR2 location, LPCBOX2 bounds,
                                                       FLOAT radius, BYTE blocked_flags, LPVECTOR2 out) {
    BOX2 rect;
    VECTOR2 clamped, nmin, nmax;
    FLOAT best_distance = FLT_MAX;
    int radius_cells, x0, x1, y0, y1;
    BOOL found = false;

    if (!location || !bounds || !out) return false;
    rect.min.x = MIN(bounds->min.x, bounds->max.x);
    rect.min.y = MIN(bounds->min.y, bounds->max.y);
    rect.max.x = MAX(bounds->min.x, bounds->max.x);
    rect.max.y = MAX(bounds->min.y, bounds->max.y);
    if (rect.max.x <= rect.min.x || rect.max.y <= rect.min.y) return false;
    clamped = (VECTOR2){
        MIN(rect.max.x, MAX(rect.min.x, location->x)),
    ...
    x0 = (int)floorf(MIN(nmin.x, nmax.x) * pathmap.width);
    x1 = (int)floorf(MAX(nmin.x, nmax.x) * pathmap.width);
    y0 = (int)floorf(MIN(nmin.y, nmax.y) * pathmap.height);
    y1 = (int)floorf(MAX(nmin.y, nmax.y) * pathmap.height);
    x0 = MIN((int)pathmap.width - 1, MAX(0, x0));
    x1 = MIN((int)pathmap.width - 1, MAX(0, x1));
    y0 = MIN((int)pathmap.height - 1, MAX(0, y0));
    y1 = MIN((int)pathmap.height - 1, MAX(0, y1));
    radius_cells = (int)ceilf(MAX(0.f, radius) / pathmap_cell_world_size());
    ...
    for (int y = y0; y <= y1; y++) {
        for (int x = x0; x <= x1; x++) {
            ...
            min_x = MAX(rect.min.x, MIN(a.x, b.x));
            max_x = MIN(rect.max.x, MAX(a.x, b.x));
            min_y = MAX(rect.min.y, MIN(a.y, b.y));
            max_y = MIN(rect.max.y, MAX(a.y, b.y));
            ...
            check_x = (int)floorf(check.x * pathmap.width);
            check_y = (int)floorf(check.y * pathmap.height);
            if (check_x != x || check_y != y) {
                /* A candidate exactly on a cell boundary can normalize into the
                 * adjacent cell. The overlap midpoint is unambiguously inside. */
