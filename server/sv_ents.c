...
static bool SV_CanClientSeeEntity(LPCCLIENT client, LPCEDICT edict) {
    ...
    if (fabs(edict->s.origin.x - clent->client->ps.server_origin.x) > VISUAL_DISTANCE)
        return false;
    if (fabs(edict->s.origin.y - clent->client->ps.server_origin.y) > VISUAL_DISTANCE)
        return false;
    return true;
#endif
}
...
#else
    edict_t *clent = client->edict;
    FLOAT dx = edict->s.origin.x - clent->client->ps.server_origin.x;
    FLOAT dy = edict->s.origin.y - clent->client->ps.server_origin.y;
